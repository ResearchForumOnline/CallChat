...
IONQ_BACKENDS = {"simulator", "qpu.forte-1"}
IONQ_FACTOR_QUBITS = 8
IONQ_FACTOR_SHOTS = 512
IONQ_FACTOR_DOMAIN = b"CallChat-IonQ-QPU-Measurement-v1"
IONQ_JOB_ID_PATTERN = re.compile(
...
def ionq_submit_factor_job(
    commitment: str,
    *,
    confirm_paid_qpu: bool = False,
    state: dict[str, Any] | None = None,
) -> tuple[dict[str, Any] | None, str | None]:
    """Submit a factor-contribution circuit without receiving client key material."""
    current = state or load_provider_state()
    details = current["providers"]["ionq"]
    key = provider_key("ionq", current)
    if not key or not details.get("enabled"):
        return None, "IonQ is not configured for this CallChat server."
    if not re.fullmatch(r"[a-f0-9]{64}", commitment):
        return None, "A valid browser entropy commitment is required."
    backend = str(details.get("backend") or "simulator")
    if backend not in IONQ_BACKENDS:
        return None, "Choose a currently supported IonQ backend."
    hardware = backend.startswith("qpu.")
    if hardware and (not details.get("allow_paid_qpu") or not confirm_paid_qpu):
        return None, "Confirm paid QPU use before submitting a hardware factor job."

    payload = {
        "type": "ionq.circuit.v1",
        "name": "CallChat QPU factor contribution",
        "backend": backend,
        "shots": IONQ_FACTOR_SHOTS,
        "metadata": {
        ...
        "input": {
            "qubits": IONQ_FACTOR_QUBITS,
            "gateset": "qis",
            "circuit": [
                {"gate": "h", "target": index}
                for index in range(IONQ_FACTOR_QUBITS)
            ],
        },
    }
    try:
        ...
        return None, f"IonQ rejected the factor job ({error.code})."
    except (OSError, urllib.error.URLError, json.JSONDecodeError):
        return None, "IonQ is temporarily unavailable."

    job_id = str(result.get("id") or "") if isinstance(result, dict) else ""
    if not IONQ_JOB_ID_PATTERN.fullmatch(job_id):
        return None, "IonQ returned an invalid factor job identifier."
    return {
        "schema": "callchat.ionq.factor-job.v1",
        "provider": "IonQ",
        "api": "v0.4",
        "backend": backend,
        "source_class": "qpu-measurement" if hardware else "simulator-test",
        "job_id": job_id,
        "status": str(result.get("status") or "submitted")[:40],
        "client_commitment": commitment,
        "qubits": IONQ_FACTOR_QUBITS,
        "requested_shots": IONQ_FACTOR_SHOTS,
        "final_factor_material": False,
    }, None


def _validated_probability_result(value: Any) -> bytes:
    if not isinstance(value, dict) or not 2 <= len(value) <= 2 ** IONQ_FACTOR_QUBITS:
        raise ProviderConfigError("IonQ returned an invalid measurement distribution.")
    normalized: dict[str, str] = {}
    total = 0.0
    for raw_outcome, raw_probability in value.items():
        outcome = str(raw_outcome)
        if not outcome.isdigit() or not 0 <= int(outcome) < 2 ** IONQ_FACTOR_QUBITS:
            raise ProviderConfigError("IonQ returned an invalid measurement outcome.")
        if isinstance(raw_probability, bool) or not isinstance(raw_probability, (int, float)):
            raise ProviderConfigError("IonQ returned an invalid measurement probability.")
        probability = float(raw_probability)
        if not math.isfinite(probability) or probability < 0.0 or probability > 1.0:
            raise ProviderConfigError("IonQ returned an invalid measurement probability.")
        normalized[str(int(outcome))] = format(probability, ".17g")
        total += probability
    if not 0.98 <= total <= 1.02:
        raise ProviderConfigError("IonQ measurement probabilities do not form a distribution.")
    return json.dumps(normalized, separators=(",", ":"), sort_keys=True).encode("ascii")
...
def ionq_factor_job(
    ...
    *,
    ...
    """Poll a factor job and extract a domain-separated measurement digest."""
    current = state or load_provider_state()
    key = provider_key("ionq", current)
    if not key or not current["providers"]["ionq"].get("enabled"):
        return None, "IonQ is not configured for this CallChat server."
    if not IONQ_JOB_ID_PATTERN.fullmatch(str(job_id or "")):
        return None, "A valid IonQ factor job identifier is required."
    if not re.fullmatch(r"[a-f0-9]{64}", commitment):
        return None, "A valid browser entropy commitment is required."

    try:
        job = request_json(
            f'{PROVIDER_ENDPOINTS["ionq"]["jobs"]}/{job_id}',
            api_key=key,
            auth_scheme="apiKey",
            timeout=PROVIDER_TIMEOUT,
            max_bytes=131_072,
        ...
        return None, f"IonQ rejected the factor status request ({error.code})."
    except (OSError, urllib.error.URLError, json.JSONDecodeError):
        return None, "IonQ is temporarily unavailable."
    if not isinstance(job, dict) or str(job.get("id") or "") != job_id:
        return None, "IonQ returned an invalid factor job response."
    status = str(job.get("status") or "")
    backend = str(job.get("backend") or current["providers"]["ionq"].get("backend") or "")
    if backend not in IONQ_BACKENDS:
        return None, "The IonQ factor job used an unsupported backend."
    metadata = job.get("metadata")
    if (
        not isinstance(metadata, dict)
        or metadata.get("purpose") != "callchat-qpu-factor-v1"
        or metadata.get("client_commitment") != commitment
        or metadata.get("security_boundary") != "no-final-factor-or-plaintext"
    ):
        return None, "The IonQ factor job does not match this browser commitment."
    source_class = "qpu-measurement" if backend.startswith("qpu.") else "simulator-test"
    base = {
        "schema": "callchat.ionq.factor-job.v1",
        "provider": "IonQ",
        "api": "v0.4",
    ...
    if status in {"submitted", "ready", "started", "running"}:
        return base, None
    if status in {"failed", "canceled"}:
        return None, f"The IonQ factor job ended with status {status}."
    if status != "completed":
        return None, "IonQ returned an unknown factor job status."

    result_url = f'{PROVIDER_ENDPOINTS["ionq"]["jobs"]}/{job_id}/results/probabilities'
    try:
        probabilities = request_json(
            result_url,
            api_key=key,
            auth_scheme="apiKey",
            timeout=PROVIDER_TIMEOUT,
        ...
        canonical_result = _validated_probability_result(probabilities)
    except ProviderConfigError as error:
        return None, str(error)
    except urllib.error.HTTPError as error:
        return None, f"IonQ rejected the factor result request ({error.code})."
    except (OSError, urllib.error.URLError, json.JSONDecodeError):
        return None, "IonQ factor results are temporarily unavailable."

    result_sha256 = hashlib.sha256(canonical_result).hexdigest()
    measurement_input = b"\0".join(
        (
            IONQ_FACTOR_DOMAIN,
    ...
    measurement_sha512 = hashlib.sha512(measurement_input).hexdigest()
    base.update(
        {
            "status": "completed",
            "completed_at": str(job.get("completed_at") or "")[:40],
            "result_outcomes": len(probabilities),
            "result_sha256": result_sha256,
            "measurement_sha512": measurement_sha512,
            "receipt": f"ionq-factor:v1:{backend}:{job_id}:{result_sha256}",
            "measurement_contribution": True,
        }
    )
    return base, None
